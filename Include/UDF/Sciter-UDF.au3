...
Func _StCreate($x = 0, $y = 0, $w = 100, $h = 50, $task = 0, $title = "Form")
	...
	Local $result = DllCall($Sciterdll, "str", "SciterClassName")
	If @error Then Return 0
	Local $ClassName = $result[0], $SciterHwnd
	If $task = 0 Then
...
Func _StIncGui($ParentGui, $x = 0, $y = 0, $w = 100, $h = 50)
	Local $result = DllCall($Sciterdll, "wstr", "SciterClassNameW")
	If @error Then Return 0
	Local $ClassName = $result[0]
	Local $SciterHwnd = _WinAPI_CreateWindowEx(0, $ClassName, "", BitOR($WS_CHILD, $WS_VISIBLE,$WS_CLIPCHILDREN), $x, $y, $w, $h,$ParentGui)
	Return $SciterHwnd
EndFunc   ;==>_StIncGui
...
Func _StLoadFile($STHwnd, $file)
	Local $result = DllCall($Sciterdll, "BOOL", "SciterLoadFile", "HWND", $STHwnd, "wstr", $file)
	If @error Then Return 0
	Return $result[0]
EndFunc   ;==>_StLoadFile
...
Func _StLoadHtml($STHwnd, $String)
	Local $StringSize = StringLen($String)
	Local $result = DllCall($Sciterdll, "BOOL", "SciterLoadHtml", "HWND", $STHwnd, "str", $String, "UINT", $StringSize, "str", @ScriptDir)
...
Func _StGetRootElement($STHwnd)
	Local $result = DllCall($Sciterdll, "int", "SciterGetRootElement", "HWND", $STHwnd, "ptr*", "")
...
Func _StGetElementHtml($el, $outer = 1)
	$sciterhtml = ""
	Local $LPCBYTE_RECEIVER = DllCallbackRegister("SciterByteCallback", "ptr", "str;UINT;ptr")
	Local $result = DllCall($Sciterdll, "int", "SciterGetElementHtmlCB", "ptr", $el, "BOOL", $outer, "ptr", DllCallbackGetPtr($LPCBYTE_RECEIVER), "ptr", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
...
Func _StSetElementHtml($el, $html, $where = 0)
	Local $htmllen = StringLen($html)
	Local $result = DllCall($Sciterdll, "int", "SciterSetElementHtml", "ptr", $el, "str", $html, "DWORD", $htmllen, "UINT", $where)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
...
Func _StGetElementText($el)
	$sciterhtml = ""
	Local $LPCWSTR_RECEIVER = DllCallbackRegister("SciterWSTRCallback", "ptr", "wstr;UINT;ptr")
	Local $result = DllCall($Sciterdll, "int", "SciterGetElementTextCB", "ptr", $el, "ptr", DllCallbackGetPtr($LPCWSTR_RECEIVER), "ptr", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
...
Func _StSetElementText($el, $String)
	...
	Local $result = DllCall($Sciterdll, "int", "SciterSetElementText", "ptr", $el, "wstr", $String, "UINT", $len)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
...
Func _StGetChildrenCount($el)
	Local $result = DllCall($Sciterdll, "int", "SciterGetChildrenCount", "ptr", $el, "UINT*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[2]
...
Func _StGetFocusElement($hwnd)
	Local $result = DllCall($Sciterdll, "int", "SciterGetFocusElement", "HWND", $hwnd, "ptr*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[2]
EndFunc
...
Func _StGetElementState($el)
	Local $result = DllCall($Sciterdll, "int", "SciterGetElementState", "ptr", $el, "UINT*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[2]
EndFunc
...
Func _StSetElementState($el, $stateToSet, $stateToClear = 0, $upt = 1)
	Local $result = DllCall($Sciterdll, "int", "SciterSetElementState", "ptr", $el, "UINT", $stateToSet, "UINT", $stateToClear, "BOOL", $upt)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
EndFunc
...
Func _StGetNthChild($el, $nth)
	Local $result = DllCall($Sciterdll, "int", "SciterGetNthChild", "ptr", $el, "UINT", $nth-1, "ptr*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[3]
...
Func _StGetParentElement($el)
	Local $result = DllCall($Sciterdll, "int", "SciterGetParentElement", "ptr", $el, "ptr*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[2]
...
Func _StGetAttributeCount($el)
	Local $result = DllCall($Sciterdll, "int", "SciterGetAttributeCount", "ptr", $el, "UINT*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[2]
...
Func _StGetNthAttribute($el, $nth)
	Local $result = DllCall($Sciterdll, "int", "SciterGetNthAttribute", "ptr", $el, "UINT", $nth, "str*", "", "wstr*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Dim $aRet[2]
	$aRet[0] = $result[3]
...
Func _StGetAttributeByName($el, $AttName)
	Local $result = DllCall($Sciterdll, "int", "SciterGetAttributeByName", "ptr", $el, "str", $AttName, "wstr*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[3]
...
Func _StSetAttributeByName($el, $AttName, $value)
	Local $result = DllCall($Sciterdll, "int", "SciterSetAttributeByName", "ptr", $el, "str", $AttName, "wstr", $value)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
...
Func _StClearAttributes($el)
	Local $result = DllCall($Sciterdll, "int", "SciterClearAttributes", "ptr", $el)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
...
Func _StGetElementIndex($el)
	Local $result = DllCall($Sciterdll, "int", "SciterGetElementIndex", "ptr", $el, "UINT*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[2]
...
Func _StGetElementType($el)
	Local $result = DllCall($Sciterdll, "int", "SciterGetElementType", "ptr", $el, "str*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[2]
...
Func _StGetStyleAttribute($el, $StyleName)
	Local $result = DllCall($Sciterdll, "int", "SciterGetStyleAttribute", "ptr", $el, "str", $StyleName, "wstr*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[3]
...
Func _StSetStyleAttribute($el, $StyleName, $StyleValue)
	Local $result = DllCall($Sciterdll, "int", "SciterSetStyleAttribute", "ptr", $el, "str", $StyleName, "wstr", $StyleValue)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
...
Func _StInsertElement($el, $elparent, $index)
	Local $result = DllCall($Sciterdll, "int", "SciterInsertElement", "ptr", $el, "ptr", $elparent, "UINT", $index)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
EndFunc
...
Func _StSelectElements($el, $CssSel)
	Local $handle = DllCallbackRegister("StElementsCallback", "BOOL", "ptr;ptr")
	Dim $aHLelementsfound[1]
	Local $result = DllCall($Sciterdll, "int", "SciterSelectElementsW", "ptr", $el, "wstr", $CssSel, "ptr", DllCallbackGetPtr($handle), "ptr", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	DllCallbackFree($handle)
	Local $HLelementsCount = UBound($aHLelementsfound)
...
Func _StSelectParent($el, $CssSel, $depth = 0)
	Local $result = DllCall($Sciterdll, "int", "SciterSelectParentW", "ptr", $el, "wstr", $CssSel, "UINT", $depth, "ptr*", "")
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return $result[4]
...
Func _StDeleteElement($el)
	Local $result = DllCall($Sciterdll, "int", "SciterDeleteElement", "ptr", $el)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
	Return 1
...
Func _StWindowAttachEventHandler($hwnd, $func, $events)
	...
	Local $result = DllCall($Sciterdll, "int", "SciterWindowAttachEventHandler", "HWND", $hwnd, "ptr", DllCallbackGetPtr($HandleWindowsAttachEvent), _
	"ptr", "", "UINT", $events)
	If @error Then Return SetError(6,0,-1)
	If $result[0] <> 0 Then Return SetError($result[0],0,-1)
...
Func HLEvHandler($tag,$el,$ev,$prm)
	Local $ap = -1
	Local $a = DllStructCreate("UINT cmd", $prm)
	Local $cmd = DllStructGetData($a, "cmd")
	$a = 0
...
Func getstructdata($str,$prm)
	Local $a = DllStructCreate($str, $prm)
	Local $b = StringSplit ( $str, ";")
	Dim $ret[$b[0]]
	For $i = 0 To $b[0]-1
...
; ===============================================================================================================
Func _StDebug()
	Local $DEBUG_OUTPUT_PROC = DllCallbackRegister("SciterDebugCallback", "ptr", "ptr;int")
	Local $result = DllCall($Sciterdll, "int", "SciterSetupDebugOutput", "ptr", "", "ptr", DllCallbackGetPtr($DEBUG_OUTPUT_PROC))
